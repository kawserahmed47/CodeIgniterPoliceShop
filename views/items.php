    ...
    <div class="row">
      ...
      <table id="example" class="table table-striped table-bordered" style="width:100%">
        ...
        <form action="<?php base_url();?>addcart1" method="POST" class="form" >
           ...
           <tr class="text-center">
              <td> <input type="radio" value="<?php echo $item->item_image ?>" name="cart_item_image">
              <input type="hidden" value="<?php echo $item->item_name ?>" name="cart_item_name">
              <input type="hidden" value="1" name="cart_item_quantity">
              <input type="hidden" value="<?php echo $item->item_type ?>" name="cart_item_type">
              <input type="hidden" value="<?php echo $item->item_price ?>" name="cart_price">
              <input type="hidden" value="<?php  echo $message ?>" name="customer_id">
               </td>
              
             <td><?php echo $item->item_name ?></td>
             <td><?php echo $item->item_type ?></td>
             <td><img width="100" height="100" src="<?php echo $item->item_image ?>" alt=""></td>
             <td><?php echo $item->item_price ?></td>
             <td>
                <a id="edit" class="btn btn-info" href="<?php base_url();?>editproduct/<?php echo $item->item_id ?>">Edit</a>
                <a class="btn btn-danger" href="<?php echo base_url();?>deleteitem/<?php echo $item->item_id ?>">Delete</a>
             </td>
           </tr>
